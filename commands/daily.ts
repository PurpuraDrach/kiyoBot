...
module.exports.dailyCommand = function(receivedMessage: any) {
  ...
  database.query(queryText, values, (err: Error, res: any) => {
    if (err) {
      console.log(err)
      receivedMessage.channel.send("There was an error in collecting dailys, please ask an Admin for help.")
      database.end()
    } else {
      if (JSON.stringify(res.rows[0].dailyavailable) == "true") {
        ...
        database.query(updateQuery, values, (err2: Error, res2: any) => {
          if (err2) {
            console.log(err2)
            receivedMessage.channel.send("There was an error in collecting quartz, please ask an Admin for help.")
            database.end()
          } else {
